...
处理 pending update（sync → 对 patched 文件做三方合并 → 更新 pin → 验证 → commit/push/close）由 `skills/self/my-skills-vendor-sync` 承担（显性调用，`/my-skills-vendor-sync`）：技能驱动全程，最终 commit、push 与关 issue 会先征求你的确认。
