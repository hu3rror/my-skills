...
upstream source and version-controlled. It is the seed source for the machine
local `~/.agents/skills` store. Editing `~/.agents/skills` directly works until
...
The repo was seeded byte-for-byte (105 files) from the current
