...
`~/.pi` repo history (removed in ticket #5, commit `bf65334`); for
`pi-extension-sync` it was untracked in `~/.pi` (removed in ticket #5) and the
`skills/self/` copy is byte-identical to the original.
